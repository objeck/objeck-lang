...
- **`JMP_TABLE` (a dense `select`) is compiled on both backends** since 2026-09-09: `sub`/`cmp`/`jae` (`b.hs`) to the default arm, then `lea`+`movsxd`+`jmp` (`adr`+`ldrsw`+`br`) through a table of 32-bit offsets placed inline after the jump. Until then neither backend whitelisted it and any method holding a `select` ran in the interpreter whole. Design: `docs/JIT_SELECT_TABLES_DESIGN.md`.

### Frame layout and the collector
A compiled frame's locals are laid out from the method's declarations (`ProcessIndices`): one native slot per declaration in id order, two for a func-ref, after the and/or temp at id 0 when the method has one. The collector (`CheckJitRoots`) walks the frame the same way, so the two must agree for every declaration, referenced or not -- laying out only the ids that instructions reference shifted every slot below an unreferenced local and had the collector read objects as arrays. Ids beyond the declarations, which no method should have, are allocated below the declared slots.
...
**Two entries, one path.** A `MTHD_CALL` whose callee is bound at compile time (anything but a `virtual` declaration) calls `JitCompiler::JitDirectCall` with the `StackMethod*` in place of the opcode; every other callback goes through `JitStackCallback`'s opcode switch. Both reach `CallCompiled`: a `virtual` declaration is first resolved through the receiver's class (`StackInterpreter::ResolveVirtualTarget`, shared with the interpreter's cold path -- before this the lookup stopped at the declaration and every virtual call from compiled code re-entered the interpreter), a callee with native code runs directly on a frame from the per-thread pool, and one without falls back to the interpreter trampoline, which is where the auto-JIT counts its calls. **A bound callee with native code is called directly** (AMD64): `EmitNativeCallFastPath` reads `StackMethod::native_entry`, builds the callee's `StackFrame` record on the caller's stack, pushes it on the call stack and calls the entry with the same values the bridge passes; the bridge sequence follows it as the slow path for a callee not compiled yet or a full call stack, and `JitNativeCallError` reports a callee's error status. The callee is entered exactly as the bridge enters it. A `virtual` site does the same through an inline cache (`JitVirtualSite`, owned by the caller's `NativeCode`): the receiver's class word against the site's current record, the record's entry on a hit, `JitResolveVirtualSite` on a miss, the bridge once a site has seen more classes than its four records. A func-ref site (`DYN_MTHD_CALL`, and the patched `DYN_MTHD_CALL_JIT`) keeps the same cache keyed by the func-ref's packed word, with `JitResolveFuncRefSite` on a miss and a two-word pop. What a call costs and the plan for the rest of it: `docs/JIT_CALLING_CONVENTION_DESIGN.md`.
