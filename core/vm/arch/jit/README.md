...
- `OBJECK_JIT_DUMP=<dir>` — AMD64 only. Writes every compiled method's native bytes to `<dir>/<sanitised-method-name>.bin` and appends a line to `<dir>/index.txt` with the method's real name, the byte length and both entry offsets (`bridge_entry`, always 0, and `native_entry`, or `-` when the method has no register-argument entry). The directory must already exist. Disassemble a blob with `objdump -D -b binary -m i386:x86-64 -M intel <dir>/<file>.bin`; start at the native entry with `--start-address=<native_entry>`, since a linear sweep from 0 describes the bridge entry, which a compiled caller does not use. Names are lossy (anything outside `[A-Za-z0-9._-]` becomes `_`, and a long signature is truncated) -- `index.txt` holds the true name. Pair it with `OBJECK_JIT_THRESHOLD=1` to compile on the first call.
