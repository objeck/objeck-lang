...
\begin{itemize}
    ...
    \item \texttt{WriteByte} - writes a byte to the socket stream.
    \item \texttt{WriteBuffer} - writes n number of bytes to the socket stream.
    \item \texttt{WriteString} - writes a string to the socket stream.
    \item \texttt{ReadByte} - reads a byte from the socket stream.
    \item \texttt{ReadBuffer} - reads n number of bytes from the socket stream.
    \item \texttt{ReadString} - reads a line from the socket stream.
\end{itemize}
