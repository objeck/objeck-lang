...
\item \texttt{Front} - returns the item at the front of the list
  ...
  \end{itemize}
\item \texttt{IsBack} - returns the item at the back of the list
  list
  \begin{itemize}
...
\item \texttt{Match} - Attempts to match input string using supplied pattern.
  Returns true if successful, false otherwise. 
  \begin{itemize}
  \item method : public : Match(input : String) \textasciitilde Bool
  \end{itemize}
\item \texttt{FindIndex} - Finds the last index of the matching pattern.
  Returns -1 if expression does not match.
  \begin{itemize}
  \item method : public : FindIndex(input : String) \textasciitilde Int
  \item method : public : FindIndex(input : String, offset : Int) \textasciitilde Int
  \end{itemize}
\item \texttt{Find} - Finds the first matching substring for the pattern.
  Returns an empty string if expression does not match.
  \begin{itemize}
  \item method : public : Find(input : String) \textasciitilde String
  \item method : public : Find(input : String, offset : Int) \textasciitilde String
  \end{itemize}
\end{itemize}
