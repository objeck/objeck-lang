...
\begin{abstract}
  Provides an introduction to the Objeck programming language and it's
  ...
  functionality that is included in release \textit{1.3.0\_1}.  For
  additional information please refer to the
  \htmladdnormallink{general}{http://sourceforge.net/projects/objeck-lang/}
  and
  \htmladdnormallink{technical}{http://rosettacode.org/wiki/Category:Objeck}
  project websites.
\end{abstract}
...
things. Features of the current release include:
\begin{itemize}
...
\begin{itemize}
\item \begin{verbatim}int DLLTools_GetArraySize(long*
    array);\end{verbatim}
\item \begin{verbatim}int DLLTools_GetFunctionValue(long* array, int
    index, FunctionId id);\end{verbatim}
\item \begin{verbatim}void DLLTools_SetFunctionValue(long* array, int
    index, FunctionId id, int value);\end{verbatim}
\item \begin{verbatim}long DLLTools_GetIntValue(long* array, int
    index);\end{verbatim}
\item \begin{verbatim}void DLLTools_SetIntValue(long* array, int
    index, long value);\end{verbatim}
\item \begin{verbatim}double DLLTools_GetFloatValue(long* array, int
    index);\end{verbatim}
\item \begin{verbatim}char* DLLTools_GetStringValue(long* array, int
    index);\end{verbatim}
\item \begin{verbatim}void DLLTools_CallMethod(DLLTools_MethodCall_Ptr
    callback, long* op_stack, long* stack_pos, long* inst, const char*
    mthd_id);\end{verbatim}
\item \begin{verbatim}long
    DLLTools_CallMethodWithReturn(DLLTools_MethodCall_Ptr callback,
    long* op_stack, long* stack_pos, long* inst, const char*
    mthd_id);\end{verbatim}
\item \begin{verbatim}void DLLTools_PushInt(long* op_stack, long
    *stack_pos, long value);\end{verbatim}
\item \begin{verbatim}void DLLTools_PushFloat(long* op_stack, long
    *stack_pos, double v);\end{verbatim}
\end{itemize}
...
\item \texttt{Filter} - applys the result of the passed in function to
  ...
  \item method : public : Filter(func : (Int/Float/Compare) $\sim$
    Bool), IntVector/FloatVector/CompareVector
  \end{itemize}
...
\subsubsection{Time}
The Time class allows programmers gain access to the current system
time.  The class supports the following operations:
\begin{itemize}
\item \texttt{New}
  \begin{itemize}
  \item New()
