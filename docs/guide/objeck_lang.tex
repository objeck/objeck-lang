...
\begin{abstract}
Objeck is an object-oriented programming language with functional
features. Objeck is a fully featured cross-platform programming language
that has ties with Java Scheme and UML.
\end{abstract}

\label{Introduction}
\chapter{Introduction}
The language was designed to be an easy to use strongly typed object oriented  
programming language. Back in 2008, when the language was under initial 
development, existing object-oriented and functional programming languages 
were examined and their syntax simplified for Objeck. 

Major language features include:
\begin{itemize}
\item Collection of rich and easy-to-use class libraries
\item Support for functional programming
\item Concurrency (threads and mutexes)
\item Automatic memory management
\item Native JIT execution (byte to machine code)
\item Unicode support with I/O support for UTF-8
\item Cross-platform static libraries
\item Command line debugger
\item Full feature support for Windows, Linux and OS X
\end{itemize}

\chapter{Getting started}
For the most recent information about obtaining binaries and source 
please refer to the project website 
\htmladdnormallink{objeck.org}{http://www.objeck.org}.
The distribution consists of a compiler, virtual machine,
debugger and library inspection tool.  The compiler executable is
named \texttt{obc}, the virtual machine (VM) 
is named \texttt{obr}.  Here is the world famous ``Hello World"
program written in the Objeck:
...
Save the above program as an ASCII or UTF-8 text file naming it  
\texttt{hello.obs}. To compile the program into an executable 
named \texttt{hello.obe} run the following command:  
...
The Objeck compiler will produce two types of binaries. One is an executable
and the other is a shared library. Shared libraries can be linked into executables 
by passing the names of libraries to the compiler. As a naming convention, 
executables must end with \texttt{*.obe} while shared libraries must end 
with \texttt{*.obl}.

Here are a few more examples. The first example compiles a program that 
processes XML. For this program, we link in the collections library as well 
as the XML parsing library.
\begin{verbatim}
obc -src examples\xml_path.obs -lib collect.obl,xml.obl -dest xml_path.obe
obr xml_path.obe
\end{verbatim}

The next example compiles a program that uses the encryption library.
...
\chapter{The basics}
Let's first of literals, variables and control flow logic.

\section{Literals and variables}

Literals are defined as they are in most programming languages. In Objeck literals are 
treated as objects and thus can have methods associated with them. As you might 
expect a character string is treated as a \texttt{String} object.
...
Examples of variable declaration and assignment are below. Variables can be explicitly 
defined by specifying their type or their types inferred through assignment or cast. 
If a variable's type is inferred it's type may cannot be redeclared.
...
\begin{tabular}{| l | l |}
  ...
  \texttt{Char} & wide character \\ \hline
  ...
  \texttt{Function} &  integer pair \\ \hline
\end{tabular}
\end{flushleft}

\section{Control flow and logic}
As with most languages, Objeck supports conditional expressions and control flow logic. 
An oddity may be that conditional statements end with semi-colons.
...
\texttt{Select} statements efficiently map integer and enum values to blocks of code.
...
Billing which supports the following looping structures \texttt{do}, 
\texttt{do/while}, \texttt{for} and \texttt{each} statements.   
...
There's support for mathematical, logical and bitwise operations.

Mathematical:
\newline
\begin{tabular}{| l | l |}
  \hline
  \emph{Operator} & \emph{Description} \\ \hline \hline
  \texttt{+} & addition \\ \hline
  \texttt{-} & subtraction \\ \hline
  \texttt{*} & multiplication \\ \hline
  \texttt{/} & division \\ \hline
  \texttt{\%} & modulus (for integer values) \\ \hline
  \texttt{<<} & shift left (for integer values) \\ \hline
  \texttt{>>} & shift right (for integer values) \\ \hline
\end{tabular}
...
\begin{tabular}{| l | l |}
  \hline
  \emph{Operator} & \emph{Description} \\ \hline \hline
  \texttt{<>} & not--equal and unary not \\ \hline
  \texttt{\&} & and \\ \hline
  \texttt{|} & or \\ \hline
  \texttt{=} & equal \\ \hline
  \texttt{<} & less--than \\ \hline
  \texttt{>} & greater--than \\ \hline
  \texttt{<=} & less--than--equal \\ \hline
  \texttt{>=} & greater--than--equal \\ \hline
\end{tabular}
...
data structures like vectors, lists, queues, etc. These classes are packaged
into bundles.
...
Please refer to the online API \htmladdnormallink{documentation}{http://www.objeck.org/docs/api/index.html} 
for additional information about bundles and classes.
...
\chapter{Appendix A: Debugging Session}
...
\chapter{Appendix B: High-Level Design}
