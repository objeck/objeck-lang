...
\begin{abstract}
  ...
  functionality that's part of v3.1.10.  For additional information
  please refer to the
...
\begin{center}
  \begin{tabular}{| l |p{4 cm} |p{4 cm} |}
    ...
    metadata & \texttt{p locl\ref \newline p @self$\rightarrow$inst\ref \newline p Klass$\rightarrow$class\ref} \\ \hline
...
\item \texttt{Find} - returns the index of the first occurrence of a
  ...
  \end{itemize}
\item \texttt{Find} - returns the index of the last occurrence of a
  given Character; -1 if not found
  \begin{itemize}
  \item method : public : native : FindLast(c : Char) \textasciitilde Int
  \item method : public : native : FindLast(offset : Int, c : Char) \textasciitilde Int
  \end{itemize}
\item \texttt{Find} - returns the index of the first occurrence of a
