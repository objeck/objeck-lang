# JIT heap corruption — investigation (CLOSED)

> **RESOLVED 2026-08-09.** Root cause was `move_freg_freg` in the ARM64 JIT
> addressing the **general** register file instead of the FP file (the
> `Register` enum restarts float numbering at `D0 = 0`), so a trailing copy in
> `cmp_mem_freg` stray-clobbered general register `X{dest}` on every float
> compare-against-memory. When that register held a live pointer, the next use
> followed a float bit pattern as an address — which is exactly the "valid
> base, valid offset, wrong VALUE" conclusion this investigation reached, and
> the `PC = 0x3F947AE147AE147B` signature (the IEEE-754 bits of `0.02`).
>
> Fixed on master (`a60371477`). Full write-up in
> `docs/VERIFY_ARM64_JIT_FLOAT_FIX.md`; guarded by
> `programs/regression/jit_float_mem_ops.obs`.
>
> **Kept because the method is reusable.** The elimination sequence below is
> what made the answer findable: it ruled out the GC, the write barrier,
> moving collection, the remembered set, UAF, stack overflow and wild store
> bases, one controlled rebuild at a time, until only "the VALUE is wrong" was
> left. The instrumentation gotchas at the end cost real time to discover and
> apply to any future JIT-emitted validator.
