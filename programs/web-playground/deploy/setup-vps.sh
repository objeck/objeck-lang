...
echo ""
echo "  4. Create the environment file the service requires:"
echo "     # playground.service has EnvironmentFile= with no '-' prefix, so systemd"
echo "     # refuses to start the unit if this file is absent."
echo "     touch /opt/playground/playground.env"
echo "     chown playground:playground /opt/playground/playground.env"
echo "     # Settings use the PLAYGROUND_ prefix. Do NOT pin PLAYGROUND_OBJECK_VERSION"
echo "     # here: it would make /api/health report a version no git pull can move."
echo ""
echo "  5. Install the Objeck toolchain and build the sandbox image:"
echo "     # update.sh downloads the published release tarball into"
echo "     # core/release/deploy (which is gitignored and empty on a fresh clone),"
echo "     # verifies its SHA256, then calls build-sandbox.sh against it. Running"
echo "     # build-sandbox.sh directly on a fresh box fails: the tree does not"
echo "     # exist yet. It must be a PUBLISHED version."
echo "     sudo -u playground bash /opt/playground/repo/programs/web-playground/deploy/update.sh"
echo ""
echo "  6. Install Caddyfile:"
