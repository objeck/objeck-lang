...
REM Evaluator/bookkeeping fixture
echo Compiling evaluator test program...
pushd "%ABS_BIN_DIR%"
"%ABS_COMPILER%" -src "%REGRESSION_DIR%\debugger_eval_test.obs" -dest "%REGRESSION_DIR%\debugger_eval_test.obe" -debug > nul 2>&1
set EVAL_COMPILE_RESULT=!errorlevel!
popd

if !EVAL_COMPILE_RESULT! neq 0 (
...
REM Multithreaded fixture
echo Compiling thread test program...
pushd "%ABS_BIN_DIR%"
"%ABS_COMPILER%" -src "%REGRESSION_DIR%\debugger_thread_test.obs" -dest "%REGRESSION_DIR%\debugger_thread_test.obe" -debug > nul 2>&1
set THREAD_COMPILE_RESULT=!errorlevel!
popd

if !THREAD_COMPILE_RESULT! neq 0 (
...
powershell.exe -ExecutionPolicy Bypass -File "%REGRESSION_DIR%\run_debugger_tests_win.ps1" -Debugger "%ABS_DEBUGGER%" -TestBin "%REGRESSION_DIR%\debugger_test.obe" -CollBin "%REGRESSION_DIR%\debugger_coll_test.obe" -EvalBin "%REGRESSION_DIR%\debugger_eval_test.obe" -ThreadBin "%REGRESSION_DIR%\debugger_thread_test.obe" -SrcDir "%REGRESSION_DIR%" -ResultsDir "%REGRESSION_DIR%\%RESULTS_DIR%"
set TEST_RESULT=!errorlevel!

REM Clean up
del /q "%REGRESSION_DIR%\debugger_test.obe" 2>nul
del /q "%REGRESSION_DIR%\debugger_coll_test.obe" 2>nul
del /q "%REGRESSION_DIR%\debugger_eval_test.obe" 2>nul
del /q "%REGRESSION_DIR%\debugger_thread_test.obe" 2>nul
