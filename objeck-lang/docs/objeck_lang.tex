...
The Objeck language toolset contains a basic interactive debugger that allows programmers to inspect variables within their programs.  The debugger will also calculate simple arithmetic expressions, which contain either variables or constants. The debugger allows programmers to set breakpoints within methods based upon the source line numbers.  The following commands are supported:

\begin{center}
\begin{tabular}{| l | l |}
\hline
\emph{Command} & \emph{Description} \\ \hline \hline
\texttt{[b]reak} &  sets a breakpoint \\ \hline
\texttt{breaks} &  shows all breakpoints \\ \hline
\texttt{[d]elete} &  deletes a breakpoint \\ \hline
\texttt{clear} &  clears all breakpoints \\ \hline
\texttt{args} &  specifies program arguments \\ \hline
\texttt{[r]un} &  runs a program that will stop at breakpoints \\ \hline
\texttt{[p]rint} &  prints the value of an expression, along with metadata \\ \hline
\texttt{[l]ist} &  lists a range of lines in a source file \\ \hline
\texttt{[i]nfo} &  displays the variables for a class or method \\ \hline
\texttt{exe} &  loads a new executable \\ \hline
\texttt{src} &  specifies a new source path \\ \hline
\texttt{[q]uit} &  exits a given debugging session \\ \hline
\end{tabular}
\end{center}

\subsection{Executing}
The source program must be compiled with the \texttt{-debug} set.The command line debugger is started up running the \texttt{odb} executable. The \texttt{-exe} option must be present and specify the path to the executable.  The The \texttt{-src} option is optional and specifies the path to the program source.  Please see the example below:

\begin{verbatim}
obd -exe test_src\hello.obe -src test_src
\end{verbatim}
