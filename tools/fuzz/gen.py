...
tell it from any other JIT access violation. The knob changes which draws a
program makes, so a finding must be replayed with the knob it was recorded
with: run_fuzz.py stores it in choices.json and reduce.py reads it back.
Choices recorded before the knob existed were made with it on.
"""
...
class Generator:
    ...
    def program(self):
        ...
        # drawn only when F5 is on, exactly as before the knob existed, so
        # choices recorded by earlier runs still replay to the same program
        drawn = self.has("F5") and self.c.chance(15)
        self.basic_lambdas = self.allow_basic_lambdas and drawn
        out = []
        out.append("#~")
