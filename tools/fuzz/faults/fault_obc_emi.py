#!/usr/bin/env python3
"""A deliberately broken compiler, to prove emi.py catches an optimizer that
...
EmiGuardZq->Dead() returns true -- the effect of a folder that wrongly decides
a runtime-false condition. s0 output is untouched, so the s3 configurations of
any variant with a dead block should split from the reference.
"""
